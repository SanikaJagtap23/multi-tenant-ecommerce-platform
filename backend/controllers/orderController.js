...
const createPaymentIntent = asyncHandler(async (req, res) => {
  ...
  const paymentIntent = await stripe.paymentIntents.create({
    amount: Math.round(grandTotal * 100), // paise / cents
    currency: "inr",
    metadata: { customerId: req.user._id.toString(), storeId },
  });

  res.json({ clientSecret: paymentIntent.client_secret, total: grandTotal });
});

// POST /api/orders
const createOrder = asyncHandler(async (req, res) => {
  const { items, storeId, shippingAddress, paymentMethod, stripePaymentId, notes } = req.body;
...
const getOrderById = asyncHandler(async (req, res) => {
  ...
  // Only the customer, the vendor, or a superadmin can view
...
const updateOrderStatus = asyncHandler(async (req, res) => {
  ...
  // Auto-mark COD as paid when vendor confirms delivery
